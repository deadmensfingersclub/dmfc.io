...
// ── 시스템 ───────────────────────────────────────────────
  system: () => `
     <div class="page-wrap">
        ...
        <p>ⓒ 명령어 및 스크립트 추가 가능 : 별도의 코딩이 필요하지 않은 <b>단순한 랜덤 스크립트 출력 명령어 추가</b>를 지원합니다. 활동 중 캐릭터 어필 등의 용도로 추가가 필요한 명령어가 있다면 아래 양식을 지켜 총괄계 DM으로 문의 바랍니다. 기존 명령어에 <b>새로운 스크립트를 추가</b>하는 것 역시 가능합니다.</p>
    <div class="species-grid">
      <div class="species-card">
        <div class="species-name">신청 양식</div>
        <div class="species-body">- 사용할 명령어<br>- 추가할 랜덤 스크립트 (최소 2개 최대 20개)</div>
      </div>
        <p>e.g. 사용할 명령어 : [오늘의 추천곡] - 추가할 랜덤 스크립트 : 오늘의 추천곡은 Goo Goo Muck입니다. / 오늘의 추천곡은 Paint It Black입니다. / …… / (최대 20개) </p>
        <p>추가된 명령어는 모든 러너가 사용할 수 있음을 유의해 주세요. 불쾌감을 유발하거나, 세계관과 어울리지 않은 제안은 거절될 수 있습니다.</p>
        <br>
        <h2>명령어 목록</h2>
      <div class="species-card">
        <div class="species-name">자판기</div>
        <div class="species-body">- 돈이 아닌 물물교환으로 작동하는 자판기로 물건을 넣으면 물건이 나옵니다.<br>
        - 자판기에 넣을 물건은 자유롭게 설정할 수 있습니다. 캐릭터가 소지하고 있을 만한 물건이라면 별도 과정 없이 자판기에 넣는다고 선언해 주시면 되지만, 캐릭터가 소지하고 있지 않은 물건을 넣는다면 스토리계를 태그하여 물건을 습득하는 역극을 진행해 습득해 주세요.<br>
        - 커뮤니티 러너가 자판기에 넣은 물건을 다른 커뮤니티 러너가 입수할 수 있습니다.<br>명령어 : <b>[자판기/투입할 물건]</b></div>
        <div class="species-ref">사용 예시: [자판기/볼펜 한 자루]</div>
        <div class="species-ref">캐릭터가 소지하고 있지 않은 물건 습득 방법의 예시 : (나는 자판기에 권총을 넣고 싶었다. 스파이시 불릿이 필요한 누군가가 있을 거라고 생각했기 때문이다. 그러나 나에게는 권총이 없기 때문에, 유라이어의 더미로 찾아가 주인장의 권총을 몰래 훔쳤다. 기다려라, 네버모어. 나의 스파이시 불릿이 간다…) @Story</div>
      </div>
      <div class="species-card">
      ...
      <div class="species-card">
        <div class="species-name">유라이어의 더미</div>
        <div class="species-body">- 유라이어의 더미에서 타로 또는 주역을 볼 수 있습니다. [유라이어의 더미] 입력 시 타로 또는 주역을 볼 수 있다는 안내메시지를 출력하며, 각 명령어를 입력할 경우 타로 또는 주역 결과를 출력합니다.<br>
        - [유라이어의 더미] 명령어의 사전 입력 없이 바로 타로나 주역을 볼 수도 있습니다.<br>
        - 커뮤니티 러너가 자판기에 넣은 물건을 다른 커뮤니티 러너가 입수할 수 있습니다.<br>명령어 : <b>[유라이어의 더미], [타로], [주역]</b></div>
      </div>
      <div class="species-card">
        <div class="species-name">주사위</div>
        <div class="species-body">- 다양한 주사위를 지원합니다. 주사위의 합연산 역시 지원하며 각 주사위의 결과를 함께 출력합니다.<br>명령어 : <b>[ndm+l]</b></div>
        <div class="species-ref">사용 예시: [1d10], [3d6+1], [2d6+1d10]</div>
      </div>
      <div class="species-card">
        <div class="species-name">성패</div>
        <div class="species-body">- 성공과 실패의 구분이 필요한 활동을 할 경우 사용합니다. <br>
        - 기본 난이도는 5이며, 5 이하는 성공, 6 이상은 실패로 구분합니다. 난이도는 가감될 수 있으며, 명령어 뒤의 숫자는 성공 기준치의 보정 값입니다. 예를 들어 [성패+2] 일 경우 7 이하의 결과를 성공으로 간주합니다.<br>명령어 : <b>[성패+n]</b></div>
        <div class="species-ref">사용 예시: [성패] [성패+2] [성패-1]</div>
      </div>
      <div class="species-card">
        <div class="species-name">선택</div>
        <div class="species-body">- 나열된 항목 중에서 랜덤으로 항목을 선택하는 기능입니다. 항목은 콤마(,)로 구분되며, 공백의 포함 여부는 무관합니다. <br>
        - 기본적으로 1개의 항목을 선택하나, n개를 선택할 수도 있습니다.<br>
        - 항목보다 선택하는 개수가 많을 경우, 중복 없이 모든 항목을 한 번씩 출력한 후 남은 횟수를 랜덤한 선택으로 채웁니다. <br>
        - 항목보다 선택하는 개수가 적을 경우, 중복 없이 n개를 선택합니다.<br>명령어 : <b>[선택/a,b,c, ...] [선택/a,b,c, … /n]</b></div>
        <div class="species-ref">사용 예시: [선택/사과, 복숭아, 딸기] [선택/사과, 복숭아, 딸기, 포도/3]</div>
      </div>
     </div>
 `,
